...
We provide a set of out of style functions out of the box through the `animus` builder from `@animus/core` that are standardized throughout all of our components. These props are strongly typed and can be included as necessary on any styled component.
