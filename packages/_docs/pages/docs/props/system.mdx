...
You may import these groups directly from `groups` method on `animus`
...
const Box = animus.groups({ positioning: true }).asComponent('div');
...
const Box = animus.groups({ shadows: true }).asComponent('div');
