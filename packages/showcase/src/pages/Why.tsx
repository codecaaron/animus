  ...
  return (
    <Stack gap={64}>
      ...
      <Stack gap={16}>
        ...
        <Prose>Two exits appeared, and both were compromises.</Prose>
        <Prose>
        ...
        <Prose>
          ...
          exist outside the TypeScript type system. Your token values are
          strings in a <InlineCode>.module.css</InlineCode> file. Your editor
          does not know them. Your variants are conditional class name
          concatenations. Dynamic values require inline styles. The compiler
          boundary severed the connection between your design decisions and the
          types that enforce them.
        </Prose>
        <Prose>
          Both approaches required developers to choose: keep the authoring
          model or keep the performance. The assumption was that you could not
          have both.
        </Prose>
      </Stack>
      ...
      <Stack gap={16}>
        ...
        <Prose>
          ...
          stylesheet. The JavaScript style definitions are tree-shaken away.
          What ships to the browser is a thin React wrapper and platform CSS —
          no serializer, no injector, no style object at runtime.
        </Prose>
        <Prose>
          This is not a runtime with a fast path. It is not a compiler that
          still ships a runtime for edge cases. Every style value must be a
          static literal resolvable at build time — the constraint is enforced
          at the type level. What crawls out of the build process is not a
          smarter runtime. It is something closer to what you would have written
          by hand if you had infinite patience and a perfect memory for
          specificity rules.
        </Prose>
        <Prose>Not a runtime. A compiler.</Prose>
      </Stack>
      ...
      <Stack gap={16}>
        ...
        <Prose>
          ...
          calculation that was designed for documents, not design systems.
          Utility frameworks respond by making every rule equally specific and
          relying on source order. Runtime libraries respond with inline styles
          and <InlineCode>!important</InlineCode>. Both are workarounds for a
          fundamental ordering problem.
        </Prose>
        <Prose>
          Animus makes the ordering explicit. Each step in the builder chain
          maps to a named CSS <InlineCode>@layer</InlineCode>. Layers have a
          fixed cascade position. When two rules conflict, the layer position
          determines the winner — not selector specificity, not source order,
          not the number of class names. There are seven layers, from lowest to
          highest precedence:
        </Prose>
        <List>
          <ListItem>
        ...
        <Prose>
          ...
          Position determines precedence. A system prop override always wins
          over a base style, regardless of how the selectors are written.{' '}
          <InlineCode>!important</InlineCode> is never needed, because the
          ordering contract is explicit and static.
        </Prose>
        <Prose>
          This is the cascade contract. You trade implicit specificity
          arithmetic for explicit layer position. The trade is always in your
          favor.
        </Prose>
      </Stack>
      ...
      <Stack gap={16}>
        ...
        <List>
          ...
          <ListItem>
            You are not using React. The builder chain produces React
            components. There is no Vue adapter, no Svelte integration, no
