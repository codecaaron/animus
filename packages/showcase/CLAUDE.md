...
- **`:focus-visible` drops when `_focusVisible` uses nested scale paths against color-mode-adaptive scales:** If `_focusVisible: { outline: '2px solid {colors.scheme.300}' }` extracts base rules but drops the `:focus-visible` selector from dist CSS, the cause is likely the interaction between a nested scale path (`{colors.scheme.300}`) and a color-mode-adaptive scale (`scheme` is a contextual-var scale, not a flat palette entry). Tracked as a known follow-on — if you see this, the `_focusVisible` block won't emit until that path is resolved.
