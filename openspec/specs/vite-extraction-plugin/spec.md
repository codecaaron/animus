...
### Requirement: External package transform exemption
Files under discovered external package directories SHALL bypass extension filters and node_modules exclusion in the transform hook. The manifest check is the authoritative gatekeeper.

#### Scenario: Transform processes external .mjs files
- **WHEN** a file's path starts with an external package directory prefix and has `.mjs` extension
- **THEN** the transform hook skips the extension check and node_modules guard — the manifest check alone determines whether to transform
...
In dev mode, edits to workspace-linked external package files SHALL trigger the HMR handler. The `node_modules` exclude pattern SHALL be bypassed for files under external package directories.
...
- **THEN** the HMR handler SHALL process it normally
