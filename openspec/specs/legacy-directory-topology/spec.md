...
Defines the repository's `legacy/` directory convention: a top-level sibling to `packages/` that holds packages previously part of the active development surface but no longer included in the publish graph or the active workspace. This capability separates "on-disk code" from "publishable code" structurally, lowers grep/LLM cognitive load during repo exploration, and enforces a one-way independence rule between active and legacy code. Legacy packages remain browsable and readable — preserved via `git mv` with history intact — but do not install, build, link, or publish.
