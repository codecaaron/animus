...
After the `e2e-workspace-topology` change is applied, `e2e/` SHALL contain `next-app/`, migrated from `packages/next-test-app/` via `git mv`.
...
- **THEN** `next-app/` is present
- **AND** it was migrated via `git mv` from `packages/next-test-app/` (history preserved as rename)
