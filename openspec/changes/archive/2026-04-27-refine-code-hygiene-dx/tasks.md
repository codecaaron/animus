...
- [x] 8.5 Test: TTY run, type `apply-all` → cascade proceeds; type `no` → exit non-zero, no cascade. [verified end-to-end on `next` branch: user typed `apply-all` at the prompt, cascade ran 2 iterations to convergence + envelope PASS + exit 0. Negative case (input ≠ "apply-all" → exit 1) is structurally identical to the verified non-TTY no-flag case. Real run also surfaced two latent issues that were fixed in same arc: (a) `WORKSPACE_DIRS[@]: unbound variable` under `set -u` in scope=all (commit `9e93680`), (b) Layer D self-deleted its own TS wrappers via knip because they're invoked from bash (commit `78bc459`).]
