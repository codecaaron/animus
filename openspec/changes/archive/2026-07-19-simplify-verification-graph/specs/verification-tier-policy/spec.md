...
**Reason**: A single-host local graph cannot reproduce GitHub runner, architecture, artifact-transfer, job-parallelism, or release-byte behavior, and the duplicated projection has drifted from CI.

**Migration**: Run `vp run verify:full` for complete local/current-host evidence and inspect or execute the owning GitHub jobs for CI-specific evidence.

## RENAMED Requirements

- FROM: `### Requirement: Root CLAUDE.md Verification Tier Table`
- TO: `### Requirement: Root AGENTS.md Verification Interface`
- FROM: `### Requirement: Vinext atomic and focused verification`
- TO: `### Requirement: Vinext focused verification`
- FROM: `### Requirement: React Router atomic and focused verification`
- TO: `### Requirement: React Router focused verification`
- FROM: `### Requirement: Worker deployment dry-run tiers`
- TO: `### Requirement: Worker deployment dry-run diagnostics`
