...
- [x] 11.11 **Generalized dist-staleness preconditions for consumer-facing tiers.** This is a systematic gap in the fail-loud precondition contract shipped by this change: `verify:build:next`, `verify:build:showcase`, and `verify:integration` all consume downstream packages through `dist/` (type definitions and/or JS), but their preconditions only check `dist/` existence — not mtime freshness against the owning package's `src/**`. The NAPI-stale check is the one-off realization of a pattern that generalizes. In practice, stale-dist leads to misleading green/red signals in consumer tiers (observed directly: an apparent successful `verify:build:next` against stale `packages/system/dist/` that reversed to a real type error after rebuild). Proposed amendment: add a second precondition helper for package-dist freshness, applied to `packages/system`, `packages/properties`, `packages/extract` (ts dist), `packages/vite-plugin`, `packages/next-plugin` wherever a downstream tier consumes them. Surfacing as a follow-up because this is a spec-level amendment that changes Requirement text on the `verification-tier-policy` capability — worth its own focused change rather than rushing into the current apply. **→ DONE-BY-FOLLOWUP**: delivered by the `dist-staleness-preconditions` change, applied 2026-04-13; see `openspec/changes/archive/<date>-dist-staleness-preconditions/` once that change is archived.
