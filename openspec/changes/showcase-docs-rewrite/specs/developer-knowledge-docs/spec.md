...
#### Scenario: Terminology consistency across all docs surfaces
- **WHEN** showcase docs pages reference the builder chain API
- **THEN** they SHALL use `.system()` (not `.groups()`), `createSystem().addGroup()` (not `.withProperties()`), and `createGlobalStyles()` (not `.withGlobalStyles()`)
