...
- [ ] 7.3 Archive via `/opsx:archive fix-selector-rule-extraction` once Phases 1–5 are complete and the fresh-build re-audit shows the CloseButton + PatternC class of regressions resolved.
- [ ] 7.4 Update session memory with the outcome: which bugs were fixed, which CopyButton-class findings were deferred to follow-on, any architectural insights worth preserving (e.g. "extractor's dev/build divergence was the bug-hiding mechanism, not the bug itself").
