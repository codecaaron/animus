...
| `vp run verify:packed`          | pack all five publishables (`bun pm pack`), per-package tarball lint (publint --strict + attw), isolated npm install into `e2e/packed-app/.staging/`, ESM/CJS + dual-engine load proof, stable-TS declaration check, Vite+Next production builds, positional assertions (`e2e/packed-app/scripts/assert-build.ts`) | fresh v1+v2 NAPI + fresh `dist/` for all five publishables + fresh `_assertions/dist/` + npm on PATH + publint/attw installed | any upstream artifact missing/stale, or npm/publint/attw absent | slow            |
...
| `vp run verify:full`     | `verify` + integration + all build + all assert tiers + packed lane                    | full local pipeline proof                        |
| `vp run verify:ci`       | best-effort mirror of CI job order + coverage (incl. packed lane mirroring its CI job) | simulate CI locally before pushing               |
...
| `packages/{properties,system,extract,vite-plugin,next-plugin}/package.json` (deps, peers, exports, files) | `verify:packed`                                                                                       |
